...
def test_cube_vc_n3_vector_count() -> None:
    vc = cube_vc(3)
    assert len(vc.vectors()) == 26


def test_cube_vc_rejects_even() -> None:
    with pytest.raises(ValueError):
        cube_vc(4)


def test_cube_vc_rejects_small() -> None:
    with pytest.raises(ValueError):
        cube_vc(1)
...
def test_cube_fan_cone_labels_in_range(fan3: Fan) -> None:
    n_vecs = len(fan3.vectors())
    for cone in fan3.cones():
        for label in cone:
            assert 0 <= label < n_vecs


def test_cube_fan_cones_are_unique(fan3: Fan) -> None:
    cones = [tuple(sorted(c)) for c in fan3.cones()]
    assert len(cones) == len(set(cones))


def test_cube_fan_rejects_even() -> None:
    with pytest.raises(ValueError):
        cube_fan(4)


def test_cube_fan_rejects_small() -> None:
    with pytest.raises(ValueError):
        cube_fan(1)
